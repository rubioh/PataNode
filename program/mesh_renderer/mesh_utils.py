...
class MeshBufferAccessor:
	...
	def __getitem__(self, index):
		...
		ret = (struct.unpack(self.char, self.buffer[index:index+self.element_size * self.element_count]))
		return ret

	def __setitem__(self, index, value):
		index = index * self.element_size * self.element_count
		self.buffer[index:index+self.element_size * self.element_count] = struct.pack(self.char. value)
...
def compute_tangent(in_indices, in_tcs, in_normals, in_position, indice_size):
	...
	for i in range(int(indices.size() / 3)):
		...
		tangent[0] = inverse_discriminant * (deltaUV2.y * edge1.x - deltaUV1.y * edge2.x);
		tangent[1] = inverse_discriminant * (deltaUV2.y * edge1.y - deltaUV1.y * edge2.y);
		tangent[2] = inverse_discriminant * (deltaUV2.y * edge1.z - deltaUV1.y * edge2.z);
		tangents[i] = tangent
