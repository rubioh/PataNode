...
@register_program(OP_CODE_JELLYFISH)
class JellyFish(ProgramBase):
    def __init__(self, ctx=None, major_version=3, minor_version=3, win_size=(960, 540)):
        super().__init__(ctx, major_version, minor_version, win_size)
        self.title = "JellyFish"
    ...
    def initProgram(self, reload=False):
        ...
        frag_path = join(dirname(__file__), "jellyfish.glsl")
...
@register_node(OP_CODE_JELLYFISH)
class JellyFishNode(ShaderNode, Scene):
    op_title = "JellyFish"
    op_code = OP_CODE_JELLYFISH
    content_label = ""
    content_label_objname = "shader_jellyfish"

    def __init__(self, scene):
        super().__init__(scene, inputs=[], outputs=[3])
        self.program = JellyFish(ctx=self.scene.ctx, win_size=(1920, 1080))
        self.eval()
