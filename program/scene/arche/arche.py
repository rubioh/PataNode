...
class Arche(ProgramBase):
    ...
    def initProgram(self, reload=False):
        ...
        frag_path = join(dirname(__file__), "arche.glsl")
...
@register_node(OP_CODE_ARCHE)
class ArcheNode(ShaderNode, Scene):
    op_title = "Arche"
    op_code = OP_CODE_ARCHE
    content_label = ""
    content_label_objname = "shader_arche"

    def __init__(self, scene):
        super().__init__(scene, inputs=[], outputs=[3])
        self.program = Arche(ctx=self.scene.ctx, win_size=(1920,1080))
        self.eval()
