...
class Cube(ProgramBase):
    ...
    def updateParams(self, fa=None):
        if fa is None:
            return
        self.time = fa['time']/2*.25
        self.smooth_fast_final = np.log(self.smooth_tmp + 1.0) * fa["bpm"] / 75 + self.deep_tic
        self.emid = (fa['low'][2] + self.smooth_fast/2) * .5
        tmp = rgb_to_hsv(self.color)
        tmp[0] += 0.002
        tmp[0] = tmp[0] % 1
        self.color = hsv_to_rgb(tmp)
        self.color_final = self.color * .7
        ...
        self.deep_tic_final = self.deep_tic * 2.
