
from os.path import dirname, join

from node.node_conf import register_node
from node.shader_node_base import ShaderNode, Utils
from program.program_base import ProgramBase
from program.program_conf import SQUARE_VERT_PATH, register_program, name_to_opcode
import math

OP_CODE_SELECTOR = name_to_opcode("selector")


@register_program(OP_CODE_SELECTOR)
class Selector(ProgramBase):
    def __init__(self, ctx=None, major_version=3, minor_version=3, win_size=(960, 540)):
        super().__init__(ctx, major_version, minor_version, win_size)
        self.title = "selector"

        self.initParams()
        self.initProgram()
        self.initFBOSpecifications()
        self.initUniformsBinding()

    def initFBOSpecifications(self):
        self.required_fbos = 1
        fbos_specification = [
            [self.win_size, 4, "f4"],
        ]

        for specification in fbos_specification:
            self.fbos_win_size.append(specification[0])
            self.fbos_components.append(specification[1])
            self.fbos_dtypes.append(specification[2])

    def initProgram(self, reload=False):
        vert_path = SQUARE_VERT_PATH
        frag_path = join(dirname(__file__), "selector.glsl")
        self.loadProgramToCtx(vert_path, frag_path, reload)

    def initParams(self):
        self.index = 0.0
        self.iChannel0 = 1

    def initUniformsBinding(self):
        binding = {"iChannel0": "iChannel0", "iResolution": "win_size"}
        self.add_float_cpu_adaptable_parameter(
            "index", self.index, lambda: 0
        )
        super().initUniformsBinding(binding, program_name="")
        self.addProtectedUniforms(["iChannel0"])

    def updateParams(self, af):
        self.index = float(
            self.getCpuAdaptableParameters()["program"]["index"]["eval_function"][
                "value"
            ]
        )

    def bindUniform(self, af):
        super().bindUniform(af)
        self.programs_uniforms.bindUniformToProgram(af, program_name="")

    def render(self, texture, af=None):
        self.bindUniform(af)
        texture[0].use(1)
        self.fbos[0].use()
        self.vao.render()
        return self.fbos[0].color_attachments[0]

    def norender(self):
        return self.fbos[0].color_attachments[0]


@register_node(OP_CODE_SELECTOR)
class SelectorNode(ShaderNode, Utils):
    op_title = "Selector"
    op_code = OP_CODE_SELECTOR
    content_label = ""
    content_label_objname = "shader_selector"

    def __init__(self, scene):
        self.last_working_index = -1
        super().__init__(scene, inputs=[1, 1, 1, 1, 1], outputs=[3])
        self.program = Selector(ctx=self.scene.ctx, win_size=(1920, 1080))
        self.eval()

    def render(self, audio_features=None):
        input_nodes = self.getShaderInputs()
        num_nodes = len(input_nodes)
        if not num_nodes or self.program.already_called:
            return self.program.norender()
        self.program.updateParams(audio_features)
        index = math.floor(float(self.program.index))
        if index < num_nodes:
            if input_nodes[index]:
                texture = input_nodes[index].render(audio_features)
                output_texture = self.program.render([texture], audio_features)
                self.last_working_index = index
                return output_texture
        if self.last_working_index  >= 0:
            index = self.last_working_index
            texture = input_nodes[index].render(audio_features)
            output_texture = self.program.render([texture], audio_features)
            return output_texture
        return self.program.norender()
