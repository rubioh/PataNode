...
class CRepet(ProgramBase):
    ...
    def updateParams(self, af=None):
        ...
        self.start_ux = self.win_size[0]//2 + np.sin(af['on_tempo32']*2.*3.14159 + af['smooth_low']*2. + self.seed)*self.win_size[1]//4
        self.size = (af['low'][2] ** 4 * 3.  + af['smooth_low'] * 2.)* 100. * (.5+.5*np.cos(af['time']*.05 + self.seed))

        self.offset_y = 100.*(af['smooth_low']**.5*5.)*np.sin(af['on_tempo2']*2.*3.14159 + self.seed)
