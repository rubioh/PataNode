...
OP_CODE_BLINK = name_to_opcode("blink")


@register_program(OP_CODE_BLINK)
class Blink(ProgramBase):
    ...
    def initProgram(self, reload=False):
        ...
        frag_path = join(dirname(__file__), "blink.glsl")
...
@register_node(OP_CODE_BLINK)
class BlinkNode(ShaderNode, Effects):
    op_title = "Blink"
    op_code = OP_CODE_BLINK
    content_label = ""
    content_label_objname = "shader_blink"

    def __init__(self, scene):
        super().__init__(scene, inputs=[1], outputs=[3])
        self.program = Blink(ctx=self.scene.ctx, win_size=(1920, 1080))
        self.eval()
