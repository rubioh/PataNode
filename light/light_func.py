...
class Kickwav:
    ...
    def kickwav(self, color, af, size):
        ...
        self.buff[:self.shift_step] *= .9
        if af['on_kick']:
            for i in range(self.shift_step):
                self.buff[i] = color * .9 ** (self.shift_step - 1 - i)

        res = np.copy(self.buff) ** 2.
        return res
    ...
    def __call__(self, color, af, size):
        ...
        if af['on_kick']:
            for spot in range(len(self.last_spots)):
                ...
                spray = np.arange(-6, 7)
                a = np.exp(- np.abs(spray) * .4).reshape(-1,1)
                self.buff[i+spray]  = color.reshape(1,-1) ** 2.2 * a ** .5
        return self.buff
    ...
    def __call__(self, color, af, size):
        ...
        if af['on_kick']:
            ...
            if self.mode == 'pingpong':
                if self.which:
                    self.buff1[0] = color*.9
                    self.buff1[1] = color
                else:
                    self.buff2[0] = color*.9
                    self.buff2[1] = color
            else:
                self.buff1[0] = color*.9
                self.buff1[1] = color
...
class All():
    ...
    def change_pattern(self):
        ...
        if self._current_pattern_index == len(self._patterns):
            self._current_pattern_index = 0
        print("changed patalight pattern to {}".format(
            ["lightKick",
             "kickwav",
             "random spots",
             "solid stars"
    ...
    def tick(self, af):
        if (datetime.now() > self._last_pattern_change + timedelta(minutes=2)) and af['mini_chill']:
