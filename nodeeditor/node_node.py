...
from nodeeditor.node_socket import Socket, LEFT_BOTTOM, LEFT_CENTER, LEFT_TOP, RIGHT_BOTTOM, RIGHT_CENTER, RIGHT_TOP, MID_TOP
    ...
    def initSockets(self, inputs: list, outputs: list, reset: bool=True):
        ...
        if 0 in inputs:
            socket = self.__class__.Socket_class(
                node=self, index=0, position=MID_TOP,
                socket_type=0, multi_edges=True,
                count_on_this_node_side=1, is_input=True
            )
            L = len(inputs)-1
